...
class PropertyInsert(ctk.CTkFrame):
    def __init__(self, parent, controller, app):
        ...
        add_property_button = ctk.CTkButton(self, text="Pridat", command=self._add_property)
        add_property_button.grid(row=11, column=0, columnspan=3, pady=20, sticky="w", padx=100)

    def _add_property(self):
        # Spusti operácie na pridaní nehnuteľnosti v novom vlákne
        threading.Thread(target=self._run_add_property).start()

    def _run_add_property(self):
        threading.Thread(target=self._add_property).start()
        # Retrieve values from input fields
        property_number = self.property_number_entry.get().strip()
        description = self.description_entry.get().strip()

        # Prístup k widgetom musí prebiehať v hlavnom vlákne, použijeme metódu after
        self.after(0, self._continue_add_property, property_number, description)

    def _continue_add_property(self, property_number, description):
        latitude_direction_1 = self.latitude_direction_var_1.get()
        latitude_value_1 = float(self.latitude_value_entry_1.get().strip())
        longitude_direction_1 = self.longitude_direction_var_1.get()
        longitude_value_1 = float(self.longitude_value_entry_1.get().strip())
        ...
        # Spustiť pridané dáta späť v hlavnom vlákne
        self.after(0, self._finalize_add_property, property_number, description, latitude_direction_1, latitude_value_1, longitude_direction_1, longitude_value_1, latitude_direction_2, latitude_value_2, longitude_direction_2, longitude_value_2)

    def _finalize_add_property(self, property_number, description, latitude_direction_1, latitude_value_1, longitude_direction_1, longitude_value_1, latitude_direction_2, latitude_value_2, longitude_direction_2, longitude_value_2):
                self.app.add_property(
            property_number,
            description,
            latitude_direction_1,
...
class PropertySearch(ctk.CTkFrame):
    def __init__(self, parent, controller, app):
        ...
        # Search Button
        search_button = ctk.CTkButton(self, text="Vyhladaj", command=self._search_properties)
        search_button.grid(row=4, column=0, columnspan=3, pady=20, sticky="w", padx=100)
    ...
    def _search_properties(self):
        threading.Thread(target=self._search_properties).start()
        latitude_direction = self.latitude_direction_var.get()
        latitude_value = self.latitude_value_entry.get().strip()
        longitude_direction = self.longitude_direction_var.get()
        longitude_value = self.longitude_value_entry.get().strip()
    ...
    def _delete_property(self, property):
        threading.Thread(target=self._delete_property, args=(property,)).start()
        if messagebox.askyesno("Potvrdenie vymazania", "Naozaj chcete vymazat tuto nehnutelnost?"):
            self.app.delete_property(property)
